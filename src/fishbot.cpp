...
// 新增：吸尘器 + 扫地刷 订阅者
rcl_subscription_t clean_sub;

// ============== 新增：超声波全局对象 ==============
Ultrasonic ultrasonic(2, 4); // Trig2, Echo4
rcl_publisher_t ultrasonic_pub;
sensor_msgs__msg__Range ultrasonic_msg;
...
// 扫地控制
void set_clean(bool enable) {
  digitalWrite(PIN_CLEAN, enable ? HIGH : LOW);
}
// 用于处理机器人上的按钮双击事件
void doubleClick()
{
...
bool setup_fishbot()
{
    ...
    // 初始化吸尘器、扫地刷引脚
    pinMode(PIN_CLEAN,  OUTPUT);

    // 上电默认开启
    digitalWrite(PIN_CLEAN,  LOW);
    clean_msg.data = true;
...
// 吸尘器回调
void clean_callback(const void *msgin) {
...
bool create_fishbot_transport()
{
    ...
    // ============== 新增：超声波初始化 ==============
    ultrasonic.begin();
    // 初始化 Range 消息（ROS2标准超声波消息）
    ultrasonic_msg.header.frame_id = micro_ros_string_utilities_set(
        ultrasonic_msg.header.frame_id, "base_ultrasonic"
    );
    // 超声波固定参数
    ultrasonic_msg.radiation_type = 1; // 1=超声波 0=红外
    ultrasonic_msg.field_of_view = 0.3; // 15度视场角
    ultrasonic_msg.min_range = 0.02;   // 最小2厘米
    ultrasonic_msg.max_range = 4.0;    // 最大4米
    delay(500);
    // 默认的内存分配器 allocator
    ...
    RCSOFTCHECK(rclc_executor_init(&executor, &support.context, 5, &allocator));
...
void callback_sensor_publisher_timer_(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    if (timer != NULL)
    {
        ...
        // ============== 新增：发布超声波数据 ==============
        float dist = ultrasonic.readDistanceMeters();
        ultrasonic_msg.header.stamp.sec = static_cast<int32_t>(stamp / 1000);              // 秒部分
        ultrasonic_msg.header.stamp.nanosec = static_cast<uint32_t>((stamp % 1000) * 1e6); // 纳秒部分
        ultrasonic_msg.range = dist;
